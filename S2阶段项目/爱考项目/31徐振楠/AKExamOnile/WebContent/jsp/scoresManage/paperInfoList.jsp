...
<script type="text/javascript">
	...
	function startExam(ma, id) {
		...
		var height = (screen.height - 450) / 2;
		// 		var url = "paper/paper_getTWBycsId?course.csId=1&up=1&paper.qtotal=0";
		var url = "classinfo/class_getClassbyInfo?course.major=" + ma
				+ "&paper.pid=" + id;
		dialog = window
				.open(
						url,
	...
	function checkPage(up, csId, ptype) {
		var url="scores/scores_getPapersByInfo?up="+up;
		if(csId!=0&&ptype!=""){
			url+="&course.csId="+csId+"&paper.ptype="+ptype;
		}
...
<body onload="closedialog()">
	<div>
		<h3 align="center">查看成绩-试卷列表</h3>
		<form action="scores/scores_getPapersByInfo?up=1" id="paperform"
			method="post">
			方向:
			...
			<s:select list="{'请选择','笔试','机试'}" name="paper.ptype" id="pType"
				class="sele" />
			&nbsp; <input type="button" value="查 询" onclick="getPaper()"
				class="buttonstyle" />
		</form>
	</div>
	<div>
		<fieldset id="marginTop">
			...
			<s:if test="%{pb.data.size()>0}">
				<div align="right" id="marginTop">
					...
					<a href="javaScript:checkPage(1,${course !=null ? course.csId:0 },'${paper != null ? paper.ptype:' ' }')">首页</a>
					&nbsp; <a
						href="javaScript:checkPage(${pb.upperpage },${course !=null ? course.csId:0 },'${paper != null ? paper.ptype:' ' }')">上一页</a>&nbsp;
					<a
						href="javaScript:checkPage(${pb.nextpage },${course !=null ? course.csId:0 },'${paper != null ? paper.ptype:' ' }')">下一页</a>&nbsp;
					<a
						href="javaScript:checkPage(${pb.pagetotal },${course !=null ? course.csId:0 },'${paper != null ? paper.ptype:' ' }')">末页</a>
				</div>
			</s:if>
		</fieldset>
	</div>
